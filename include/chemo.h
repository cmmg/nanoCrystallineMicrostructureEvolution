  ...
  for(unsigned int q=0;q<n_q_points;q++){
	...
	for(unsigned int B=0;B<dofs_per_cell;B++){
	  ...
	  if(cb==ca){	  
	    ...
	    for(unsigned int p=0;p<n_diff_grains;p++){
	      if(p==cb)continue;
	      else sq_sum+=std::pow((c[q][p]),2);
	    }
	    local_matrix(A,B)+= fe_values.shape_value(A,q)*fe_values.shape_value(B,q)*((1.0/dt) + L1*(-alpha1+3*beta1*pow((c_conv[q][ca]),2)+2*gamma1*sq_sum))*fe_values.JxW(q);
	  }
	}
      }
    }
